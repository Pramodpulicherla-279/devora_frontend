/* Lesson: Correlation and Covariance
...
// generate a scatter with target correlation r
function scatter(r, n = 40) {
  const pts = [];
  for (let i = 0; i < n; i++) {
    const x = Math.random();
    const noise = Math.random() - 0.5;
    const y = r * (x - 0.5) + (1 - Math.abs(r)) * noise * 1.0 + 0.5;
    ...
    <div className="dscc-wrap">
      ...
      <div className="dscc-chart-wrap">
        <svg viewBox={`0 0 ${W} ${H}`} className="dscc-svg" preserveAspectRatio="xMidYMid meet">
          ...
          {Math.abs(r) > 0.1 && (
            <line x1={X(0)} y1={Y(0.5 + r * -0.5)} x2={X(1)} y2={Y(0.5 + r * 0.5)} className="dscc-trend" />
          )}
          {pts.map((p, i) => <circle key={i} cx={X(p.x)} cy={Y(p.y)} r="3.5" className="dscc-pt" />)}
          <text x={W / 2} y={H - 4} className="dscc-axis-label">variable X →</text>
        </svg>
      </div>
      ...
      <div className="dscc-readout" style={{ '--rc': r > 0.1 ? '#56d364' : r < -0.1 ? '#f0883e' : '#7d8590' }}>
        <strong>{strength}</strong> {dir} correlation
      </div>
      <div className="dscc-note">
        <strong>r</strong> ranges −1…+1 (direction &amp; strength, unit-free). <strong>Covariance</strong> is the same idea but in the data's raw units, so it's harder to compare. ⚠️ Correlation ≠ causation.
      </div>
    </div>
  );
};
