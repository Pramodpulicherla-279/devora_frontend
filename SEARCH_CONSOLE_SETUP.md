...
The sitemap is **dynamically generated by the backend** and includes:

- **Static pages**: Homepage (`/`), Terms (`/terms`), Privacy (`/privacy-policy`)
- **Course pages**: All available courses from the database (e.g., `/course/html/introduction-to-html`)
- **Lesson pages**: Individual lessons within each course (e.g., `/course/html/introduction-to-html`)

### Backend Sitemap Generation

The sitemap is served directly from the backend at `https://devora-backend.onrender.com/sitemap.xml` and is automatically redirected when you access `https://www.dev-el.co/sitemap.xml`.

**Benefits of backend generation:**
- Always up-to-date with the latest courses and lessons from the database
- Includes accurate `lastmod` dates from the database
- No need to rebuild the frontend to update the sitemap
- Real-time reflection of content changes

### Frontend Sitemap Fallback (Optional)

For development or if the backend is unavailable, you can generate a static sitemap on the frontend:

```bash
npm run generate-sitemap
```

This creates a fallback sitemap in `/public/sitemap.xml` with:
- Static pages
- Hardcoded course pages
- Falls back when API is unreachable

**Note**: In production, the backend-generated sitemap is used via the Firebase redirect in `firebase.json`.
...
2. The backend sitemap is **automatically updated in real-time**
3. No rebuild or redeployment needed
4. New pages are immediately discoverable by search engines

The sitemap updates instantly with no manual intervention required!
